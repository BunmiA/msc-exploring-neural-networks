...
late_fusion_model_batch = tf.keras.Model(inputs=[train_clip_start, train_clip_end],
                                       outputs=label)
...
late_fusion_model_batch.summary()

sgd = tf.keras.optimizers.SGD(lr=1e-3, momentum=0.9, decay=0.0005)

late_fusion_model_batch.compile(optimizer=sgd, loss='sparse_categorical_crossentropy', metrics=['accuracy'])

late_fusion_model_batch.fit(d_batch,
                          epochs=20,
                          verbose=1,
                          callbacks=[callbacks])

print('\n# Evaluate on test data')
results = late_fusion_model_batch.evaluate(d_batch_test)
