...
vgg_late_fusion_model = tf.keras.Model(inputs=[train_clip_start, train_clip_end],
                                           outputs=label)
...
print('\n# creating late fusion model using vgg pretrained model')
vgg_late_fusion_model.summary()

sgd = tf.keras.optimizers.SGD(lr=0.1, momentum=0.9, decay=0.0005)

vgg_late_fusion_model.compile(optimizer=sgd, loss='sparse_categorical_crossentropy', metrics=['accuracy'])

vgg_late_fusion_model.fit(d_batch,
                              epochs=5,
                              verbose=1,
                              callbacks=[callbacks])
...
results = vgg_late_fusion_model.evaluate(d_batch_test)
