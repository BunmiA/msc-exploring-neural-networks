...
train_clip_1_features = l.MyLRNLayer()(train_clip_1_features)
...
train_clip_2_features = l.MyLRNLayer()(train_clip_2_features)
...
train_clip_3_features = l.MyLRNLayer()(train_clip_3_features)
...
train_clip_4_features = l.MyLRNLayer()(train_clip_4_features)
...
first_half = l.MyLRNLayer() (first_half)
...
second_half = l.MyLRNLayer()(second_half)
...
slow_fusion_model_lrn = tf.keras.Model(inputs=[train_clip_1, train_clip_2, train_clip_3, train_clip_4],
                                      outputs=label)
...
print('\n# creating slow fusion model using local response normalization')
slow_fusion_model_lrn.summary()

sgd = tf.keras.optimizers.SGD(lr=1e-3, momentum=0.9, decay=0.0005)

slow_fusion_model_lrn.compile(optimizer=sgd, loss='sparse_categorical_crossentropy', metrics=['accuracy'])

slow_fusion_model_lrn.fit(d_batch,
                         epochs=20,
                         verbose=1,
                         callbacks=[callbacks])

print('\n# Evaluate on test data')
results = slow_fusion_model_lrn.evaluate(d_batch_test)
