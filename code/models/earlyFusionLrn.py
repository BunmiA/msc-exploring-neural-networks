...
print('\n# creating early fusion model using Local response normalization')
model_early_fusion_lrn.summary()

sgd = tf.keras.optimizers.SGD(lr=1e-3, momentum=0.9, decay=0.0005)

model_early_fusion_lrn.compile(optimizer=sgd, loss='sparse_categorical_crossentropy', metrics=['accuracy'])
...
results = model_early_fusion_lrn.evaluate(test_multi_frame_T_batches)
