...
train = train.map(f.select_random_frame)
train = train.map(f.convert_to_tuple)

test = ucf101_test.map(f.format_videos)
test = test.map(f.select_random_frame)
...
adam = tf.keras.optimizers.Adam(lr=0.001, beta_1=0.9, beta_2=0.999, epsilon=10^-8, decay=0.0, amsgrad=False)
